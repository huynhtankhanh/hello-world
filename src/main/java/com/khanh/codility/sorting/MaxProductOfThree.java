...
public class MaxProductOfThree {
	 ...
	 * (P, Q, R) equates to A[P] * A[Q] * A[R] (0 le P lt Q lt R lt N).
	 * 
	 ...
	 * 
	 * (0, 1, 2), product is minus 3 * 1 * 2 = minus 6 (1, 2, 4), product is 1 * 2 *
	 * 5 = 10 (2, 4, 5), product is 2 * 5 * 6 = 60 Your goal is to find the maximal
	...
	public int solution(int[] A) {
		...
		int length = A.length;
		int solution1 = A[length - 1] * A[length - 2] * A[length - 3];

		int solution2 = 0;
		if (A[0] < 0 && A[1] < 0) {
			solution2 = A[0] * A[1] * A[length - 1];
		}
